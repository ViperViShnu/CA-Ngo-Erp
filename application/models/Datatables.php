...
class Datatables extends MY_Model
{
    ...
    public function get_paymentReceipt_report($filterBy = null, $type = null)
    {
        $where = [];

        if (!empty($filterBy) && $type == 'by_account') {
            $where['tbl_transactions.account_id'] = $filterBy;
        } elseif (!empty($filterBy) && $type == 'by_paid_out_of') {
            // $where = "(category_id = '{$filterBy}' OR income_category_id = '{$filterBy}')"; // Raw condition
            $where = "(type = 'Income' AND category_id = '{$filterBy}' OR type = 'Expense' AND income_category_id = '{$filterBy}')"; // 2025-03-18
        }

        $all_transactions_report = array_reverse($this->get_datatable_permissionNew(null, $where, null));
        
        return $all_transactions_report;
    }

    public function get_paymentReceipt_reportOld($filterBy = null, $type = null)
    {
        ...
        if(!empty($filterBy) && $type == 'by_account') {
            ...
            $this->db->where("(category_id = '{$filterBy}' OR income_category_id = '{$filterBy}')", null, false);
        }
        ...
        // if (!empty($filterBy) && $type == 'by_account') {
        //     $where['tbl_transactions.account_id'] = $filterBy;
        // } else if (!empty($filterBy) && $type == 'by_paid_out_of') {
        //     $where['category_id'] = $filterBy;
        //     $where['income_category_id'] = " OR". $filterBy;
        //     // $where[] = "(category_id = '{$filterBy}' OR income_category_id = '{$filterBy}'";
        //     // $where = array('category_id' => $filterBy, 'income_category_id' => $filterBy);
        // }
        // $all_transactions_report = array_reverse($this->get_datatable_permission($where));
        // echo "<pre>"; print_r($this->db->last_query()); die;
        // return $all_transactions_report;
    ...
    public function get_all_clock_history()
    {
        ...
        if (!empty($all_clock_history)) {
            foreach ($all_clock_history as $clock_history) {
                ...
                $attendance_info = get_row('tbl_attendance', array('attendance_id' => $clock_info->attendance_id));
                $clock_history->date_in = (!empty($attendance_info->date_in) ? $attendance_info->date_in : '-');
                $clock_history->date_out = (!empty($attendance_info->date_out) ? $attendance_info->date_out : '-');
            }
        }
    ...
    public function get_datatable_permissionNew($user_id = null, $where = null, $where_in = null)
    {
        if (empty($user_id)) {
            $user_id = $this->session->userdata('user_id');
        }

        $this->make_query(); // Ensure this method correctly sets up the query
        ...
        if (!empty($where)) {
            if (is_array($where)) {
                ...
                $this->db->where($where, null, false); // Ensure this is correctly formatted SQL
            }
        }

        // Apply WHERE IN condition
        if (!empty($where_in) && is_array($where_in) && count($where_in) == 2) {
            $this->db->where_in($where_in[0], $where_in[1]);
        }
        ...
        // **Debugging the SQL query before returning**
        if (!$query) {
            echo "<pre>SQL Error: " . $this->db->last_query() . "</pre>";
            echo "<pre>DB Error: " . $this->db->error()['message'] . "</pre>";
            die;
        }
