...
class Admin_Controller extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        switch ($case) {
            ...
            default:
                ...
                $exists = $this->db->where(array('label' => 'liability_report', 'status' => 1))->get('tbl_menu');
                if($exists->num_rows() == 0) {
                    $reportMenu = $this->db->where(array('parent' => 146))->get('tbl_menu')->result();

                    foreach($reportMenu as $menu) {
                        if($menu->sort != 0 && $menu->sort != 1 && $menu->sort != 2) {
                            $this->db->where(array('menu_id' => $menu->menu_id))->update('tbl_menu', array('sort' => $menu->sort + 2));
                        }
                    }
                    $this->lLiabilityAndAssetReports();
                }
                $this->checkColumnExists('tbl_transactions', 'income_category_id', 'INT', '11', true, NULL);
                $this->checkColumnExists('tbl_transactions', 'previous_total_balance', 'DECIMAL', '18,2', false, 0.00);
                break;
        }
    ...
    public function lLiabilityAndAssetReports() {
        $data = [
            [
                'label' => 'liability_report',
                'link' => 'admin/report/liability_report',
                'icon' => 'fa fa-circle-o',
                'parent' => 146,
                'sort' => 3,
                'status' => 1,
            ...
            [
                'label' => 'asset_report',
                'link' => 'admin/report/asset_report',
                'icon' => 'fa fa-circle-o',
                'parent' => 146,
                'sort' => 4,
                'status' => 1,
            ...
            [
                ...
                'link' => 'admin/report/payment_receipt',
                'icon' => 'fa fa-circle-o',
                'parent' => 146,
                'sort' => 11,
                'status' => 1,
            ...
            [
                'label' => 'receipt_payment_reports',
                'link' => 'admin/report/receipt_payment',
                'icon' => 'fa fa-circle-o',
                'parent' => 146,
                'sort' => 12,
                'status' => 1,
        ...
        $this->db->insert_batch('tbl_menu', $data);
    }

    public function checkColumnExists($table, $column, $type, $constraint, $is_null, $default) {
        $this->load->database();
        $this->load->dbforge();
