...
class Report extends Admin_Controller
{
    ...
    public function paymentReceiptList($filterBy = null, $type = null)
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('datatables');
            $this->datatables->table = 'tbl_transactions';
            ...
            // get all invoice
            $this->load->model('transactions_model');
            $fetch_data = $this->datatables->get_paymentReceipt_report($filterBy, $type);
            ...
            foreach ($fetch_data as $_key => $v_transaction) {
                ...
                $sub_array = array();
                // $sub_array[] = $v_transaction->transactions_id;
    ...
    public function paymentReceiptTotalList() {
        $filterBy = $this->input->post('filterBy');
        $type = $this->input->post('type');

        $this->db->order_by('transactions_id', 'DESC');
        if(!empty($filterBy) && $type == 'by_account') {
            $this->db->where('tbl_transactions.account_id', $filterBy);
        } elseif(!empty($filterBy) && $type == 'by_paid_out_of') {
            // $this->db->where(array('type' => 'Income', 'category_id' => $filterBy))->or_where(array('type' => 'Expense', 'income_category_id' => $filterBy));
            $this->db->group_start();
            $this->db->where('type', 'Income')->where('category_id', $filterBy);
            $this->db->or_group_start();
            $this->db->where('type', 'Expense')
                ->where('income_category_id', $filterBy);
            $this->db->group_end();
            $this->db->group_end();
        }
        ...
        $total_amount = $total_credit = $total_debit = $total_balance = 0;

        foreach($all_transaction_info as $data) {
        ...
        $data = array(
            'total_amount' => display_money($total_amount, default_currency()),
            'total_credit' => display_money($total_credit, default_currency()),
            'total_debit' => display_money($total_debit, default_currency()),
            'total_balance' => display_money($total_balance, default_currency()),
        );

        echo json_encode(array('success' => true, 'data' => $data)); die;
    }
    ...
    public function date_wise_report()
    {
        ...
        if (!empty($data['start_date']) && !empty($data['end_date'])) {
            ...
            $data['all_transaction_info'] = $this->db->where(array('date >=' => $data['start_date'], 'date <=' => $data['end_date']))->get('tbl_transactions')->result();
        }
    ...
    {
        $data['title'] = lang('payment_receipt');
        $data['subview'] = $this->load->view('admin/report/payment_receipt', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    ...
    public function sales_report($filterBy = null)
    {
        ...
        if (!empty($filterBy)) {
            ...
            if ($filterBy == 'invoices' || $filterBy == 'invoice_by_client' || $filterBy == 'pos_sales') {
                $start_date = null;
                $end_date = null;
                $notes = null;
                ...
                } elseif($filterBy == 'pos_sales') {
                    $notes = 'Directly from purchase';
                    $status = 'Directly from purchase';
                } else {
                    $status = 'all';
                }
                if ($this->input->post()) {
                ...
                $data['all_invoices'] = $this->invoice_model->get_invoice_report($data['status'], $range, $data['notes']);
    ...
    public
    function sales_report_pdf($filterBy = null, $status = null, $start_date = null, $end_date = null)
    {
    ...
    public function liability_report() {
        $data['title'] = lang('liability_report');
        $data['start_date'] = $this->input->post('start_date', TRUE);
        $data['end_date'] = $this->input->post('end_date', TRUE);
        if(!empty($data['start_date']) && !empty($data['end_date'])) {
            $data['report'] = TRUE;
            ...
            $expense_categories = array(11); // Expense expense_category_id to get those specific category id only in the Liabilities table...
            $data['all_transaction_info'] = $this->db->where('date >= ', $data['start_date'])->where('date <= ', $data['end_date'])
                ->group_start()
        ...
        $data['subview'] = $this->load->view('admin/report/liability_reportNew', $data, TRUE);
        $this->load->view('admin/_layout_main', $data);
    }

    public function liability_report_pdf($start_date, $end_date)
    {
        $data['title'] = lang('liability_report');
        $data['start_date'] = $start_date;
        $data['end_date'] = $end_date;
        ...
        $income_categories = array(4);
        $expense_categories = array(11);
        $data['all_transaction_info'] = $this->db->where('date >= ', $data['start_date'])->where('date <= ', $data['end_date'])
            ->group_start()
                ->where('type', 'Income')->where_in('category_id', $income_categories)
                ->or_group_start()
        ...
        $viewfile = $this->load->view('admin/report/liability_report_pdf', $data, TRUE);
        pdf_create($viewfile, slug_it(lang('liability_report')));
    }

    public function asset_report() {
        $data['title'] = lang('asset_report');
        $data['start_date'] = $this->input->post('start_date', TRUE);
        $data['end_date'] = $this->input->post('end_date', TRUE);
        if(!empty($data['start_date']) && !empty($data['end_date'])) {
            $data['report'] = TRUE;

            $expense_categories = array(12); // Expense expense_category_id to get those specific category id only in the Assets table...
            $data['all_transaction_info'] = $this->db->where('date >= ', $data['start_date'])->where('date <= ', $data['end_date'])
                ->group_start()
                    ->where('type', 'Expense')->where_in('category_id', $expense_categories)
                ->group_end()
        ...
        $data['subview'] = $this->load->view('admin/report/asset_report', $data, TRUE);
        $this->load->view('admin/_layout_main', $data);
    }

    public function asset_report_pdf($start_date, $end_date)
    {
        $data['title'] = lang('asset_report');
        $data['start_date'] = $start_date;
        $data['end_date'] = $end_date;
        ...
        $viewfile = $this->load->view('admin/report/asset_report_pdf', $data, TRUE);
        pdf_create($viewfile, slug_it(lang('asset_report')));
    }

    public function receipt_payment()
    {
        $data['title'] = lang('receipt_payment_reports');
        $data['start_date'] = $this->input->post('start_date', TRUE);
        $data['end_date'] = $this->input->post('end_date', TRUE);
        if (!empty($data['start_date']) && !empty($data['end_date'])) {
            $data['receipts_info'] = $this->db->where(array('type' => 'Income', 'date >= ' => $data['start_date'], 'date <= ' => $data['end_date']))->order_by('date')->get('tbl_transactions')->result();

            $data['payments_info'] = $this->db->where(array('type' => 'Expense', 'date >= ' => $data['start_date'], 'date <= ' => $data['end_date']))->order_by('date')->get('tbl_transactions')->result();

            if(!empty($data['receipts_info']) || !empty($data['payments_info'])) {
                $data['report'] = true;
            }
        }
        $data['subview'] = $this->load->view('admin/report/receipt_payment_reports', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }

    public function receipt_payment_report_pdf($start_date, $end_date)
    {
        $data['title'] = lang('receipt_payment_report');
        $data['start_date'] = $start_date;
        $data['end_date'] = $end_date;
        $this->load->helper('dompdf');
        $data['receipts_info'] = $this->db->where(array('type' => 'Income', 'date >= ' => $data['start_date'], 'date <= ' => $data['end_date']))->order_by('date')->get('tbl_transactions')->result();
        $data['payments_info'] = $this->db->where(array('type' => 'Expense', 'date >= ' => $data['start_date'], 'date <= ' => $data['end_date']))->order_by('date')->get('tbl_transactions')->result();
        $viewfile = $this->load->view('admin/report/receipt_payment_report_pdf', $data, TRUE);
        pdf_create($viewfile, slug_it(lang('receipt_payment_report')));
    }
}