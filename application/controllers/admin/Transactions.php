...
class Transactions extends Admin_Controller
{
    ...
    {
        $data['title'] = lang('all_deposit');        // get permission user by menu id
        $data['active'] = 1;
        $data['subview'] = $this->load->view('admin/transactions/deposit', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }

    public function create_deposit($id = NULL)
    {
        ...
        $data['active'] = 2;
        $data['dropzone'] = true;
        $data['subview'] = $this->load->view('admin/transactions/create_deposit', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    ...
    public function transaction_deposit_state_report()
    {
        ...
        $data = array();
        $pathonor_jonno['transaction_deposit_state_report_div'] = $this->load->view("admin/transactions/transaction_deposit_state_report", $data, true);
    ...
    public function save_imported()
    {
        ...
        foreach ($files as $key => $value) {
            if (!empty($value)) {
                ...
                $data['notes'] = trim($value[2]);
                $data['reference'] = trim($value[3]);
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            ...
            if (!empty($data['amount'])) {
                if (!empty($id)) {
                    ...
                    $data['previous_total_balance'] = $account_info->balance;
    ...
    public function create_expense($id = NULL)
    {
        $data['title'] = lang('all_expense');
        // get permission user by menu id
        $data['permission_user'] = $this->transactions_model->all_permission_user('31');
        ...
        $data['subview'] = $this->load->view('admin/transactions/create_expense', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    ...
    public function expenseList($filterBy = null, $type = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $custom_field = custom_form_table_search(2);
            $action_array = array('transactions_id');
            $main_column = array('transactions_id', 'transaction_prefix', 'tbl_transactions.name', 'date', 'amount', 'notes', 'tbl_accounts.account_name', 'tags', 'status', 'reference');
            $result = array_merge($main_column, $custom_field, $action_array);
            $this->datatables->column_order = $result;
            $this->datatables->column_search = $result;
            ...
            foreach ($fetch_data as $_key => $v_expense) {
                ...
                $transaction_prefix = null;
                $transaction_prefix .= '<a class="text-info" href="' . base_url() . 'admin/transactions/view_details/' . $v_expense->transactions_id . '">' . (!empty($v_expense->transaction_prefix) ? $v_expense->transaction_prefix : ' ') . '</a>';
                $sub_array[] = $transaction_prefix;
    ...
    public function transactions_state_report()
    {
        ...
        $data = array();
        $pathonor_jonno['transactions_state_report_div'] = $this->load->view("admin/transactions/transactions_state_report", $data, true);
        echo json_encode($pathonor_jonno);
        exit;
    }
    ...
    public function save_expense($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            $data = $this->transactions_model->array_from_post(array(
                'transaction_prefix', 'name', 'date', 'notes', 'category_id', 'paid_by', 'tags', 'payment_methods_id', 'reference', 'project_id', 'billable', 'client_visible',
                'repeat_every', 'total_cycles', 'done_cycles', 'income_category_id'
            ));

            if(!empty($data['income_category_id'])) {
                $income_category_info = $this->db->where('income_category_id', $data['income_category_id'])->get('tbl_income_category')->row();

                $this->db->where(array('type' => 'Income', 'category_id' => $data['income_category_id']));
                if(!empty($id)) {
                    $this->db->where('transactions_id != ', $id);
                }
                $total_income = $this->db->select_sum('amount')->get('tbl_transactions')->row()->amount ?? 0;

                $this->db->where(array('type' => 'Expense', 'income_category_id' => $data['income_category_id']));
                if(!empty($id)) {
                    $this->db->where('transactions_id != ', $id);
                }
                $total_expense = $this->db->select_sum('amount')->get('tbl_transactions')->row()->amount ?? 0;

                $total_balance = $total_income - $total_expense;

                if($total_balance < $this->input->post('amount', TRUE)) {
                    set_message('error', 'The expense amount exceeds the ' . $income_category_info->income_category . ' amount, The available balance in '. $income_category_info->income_category . ' is '. $total_balance);
                    redirect('admin/transactions/create_expense');
                }
            }
        ...
        if (!empty($data['project_id']) && is_numeric($data['project_id'])) {
            redirect('admin/projects/project_details/' . $data['project_id'] . '/' . 'expense');
        } else {
            redirect('admin/transactions/expense');
        }
    }
    ...
    public function view_expense($id)
    {
        ...
        if (!empty($data['expense_info']) && $data['expense_info']->type == 'Income') {
            ...
            $data['id'] = $id;
            $data['all_tabs'] = transactions_details_tabs($id);
            $data['title'] = $type . ' ' . lang('details');
    ...
    public function view_details($id, $active = null)
    {
        $data['expense_info'] = $this->db->where('transactions_id', $id)->get('tbl_transactions')->row();
        // $data['title'] = $type . ' ' . lang('details');
    ...
    public function create_transfer($id = NULL)
    {
        $data['title'] = lang('transfer');
        // get permission user by menu id
        $data['permission_user'] = $this->transactions_model->all_permission_user('32');
        ...
        $data['dropzone'] = true;
        $data['subview'] = $this->load->view('admin/transactions/create_transfer', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    ...
    public function transferList($filterBy = null, $type = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($fetch_data as $_key => $v_transfer) {
                ...
                if (!empty($can_edit) && !empty($edited)) {
                    $action .= btn_edit('admin/transactions/create_transfer/' . $v_transfer->transfer_id) . ' ';
                }
                if (!empty($can_delete) && !empty($deleted)) {
    ...
    public function save_transfer($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            ...
            if ($to_account_id == $from_account_id) {
                ...
                if (!empty($upload_file)) {
                    $transfer_data['attachement'] = json_encode($upload_file);
                } else {
    ...
    public function download($id, $fileName = null)
    {
        ...
        if (!empty($fileName)) {
            $attachment = json_decode($file_info->attachement);
            ...
            if (!empty($attachement_info)) {
                $total = count(array($attachement_info));
    ...
    public function checkAvailableBalance() {
        if($this->input->is_ajax_request()) {
            $data = $this->transactions_model->array_from_post(array('transactions_id', 'income_category_id', 'amount'));
            if(!empty($data['income_category_id'])) {
                ...
                if(!empty($data['transactions_id'])) {
                    $this->db->where('transactions_id != ', $data['transactions_id']);
                }
                $total_income = $this->db->select_sum('amount')->get('tbl_transactions')->row()->amount ?? 0;
                ...
                if($total_balance < $data['amount']) {
                    $message = 'The expense amount exceeds the ' . $income_category_info->income_category . ' amount, The available balance in '. $income_category_info->income_category . ' is '. $total_balance;
                    echo json_encode(array("success" => false, "message" => $message)); die;
                } else {
                    echo json_encode(array("success" => true, "message" => 'value')); die;
                }
            }
        }
    }
