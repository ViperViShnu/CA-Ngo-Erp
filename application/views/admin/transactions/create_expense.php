                    ...
                    <div class="tab-pane <?= $active == 2 ? 'active' : ''; ?>" id="create">
                        <form role="form" data-parsley-validate="" novalidate="" enctype="multipart/form-data"
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-lg-4">
                                    <div class="input-group">
                                        ...
                                        <div class="input-group-addon"
                                            title="<?= lang('new') . ' ' . lang('expense_category') ?>"
                            ...
                            <div class="form-group">
                                ...
                                <label class="col-lg-2 control-label"><?= lang('paid_out_of') ?> </label>
                                <div class="col-lg-4">
                                    <div class="input-group">
                                        <select <?php
                                                    if (!empty($project)) {
                                            ...
                                            <option value="0"><?= lang('none') ?></option>
                                            <?php
                                                $income_category_info = $this->db->order_by('income_category_id', 'DESC')->get('tbl_income_category')->result();
                                                if (!empty($income_category_info)) {
                                                    foreach ($income_category_info as $v_category) {
                                                ?>
                                            <option value="<?= $v_category->income_category_id ?>" <?php
                                                                                                                if (!empty($expense_info->income_category_id)) {
                                                                                                                    echo $expense_info->income_category_id == $v_category->income_category_id ? 'selected' : '';
                                                                                                                }
                                                                                                                ?>>
                                        ...
                                        <?php if (!empty($created)) { ?>
                                        <div class="input-group-addon"
                                            title="<?= lang('new') . ' ' . lang('income_category') ?>"
                                            data-toggle="tooltip" data-placement="top">
                                            <a data-toggle="modal" data-target="#myModal"
                            ...
                            <div class="form-group mt-lg">
                                <label for="field-2" class="col-lg-2 control-label"><?= lang('attachment') ?></label>
                                <div class="col-lg-10">
...
<script type="text/javascript">
    function validateAmount() {
        var category_id = $('select[name="income_category_id"]').val();
        var amountField = $('input[name="amount"]:not([type="hidden"])');
        ...
        if (category_id !== 'undefined' && amount !== 'undefined') {
            $.ajax({
                url: base_url + "admin/transactions/checkAvailableBalance",
                type: "POST",
                dataType: "json",
                data: {
                    transactions_id: id,
                ...
                success: function(response) {
                    if (!response.success) {
                        amountField.after('<span class="amount-error text-danger">' + response.message + '</span>');
    ...
    $('select[name="income_category_id"], input[name="amount"]:not([type="hidden"])').on('change keyup', validateAmount);
</script>