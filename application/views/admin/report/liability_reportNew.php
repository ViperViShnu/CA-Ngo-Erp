<form role="form" enctype="multipart/form-data" id="form"
      action="<?php echo base_url(); ?>admin/report/liability_report" method="post" class="form-horizontal  ">
                ...
                <div class="panel-title">
                    ...
                    if (!empty($all_transaction_info)):
                        ...
                        <div class="pull-right hidden-print">
                            <a href="<?php echo base_url() ?>admin/report/liability_report_pdf/<?= date('Y-m-d', strtotime($start_date)) . '/' . date('Y-m-d', strtotime($end_date)) ?>"
            ...
            <div class="table-responsive">
                <table class="table table-striped DataTables " id="" cellspacing="0" width="100%">
                    ...
                    if (!empty($all_transaction_info)): foreach ($all_transaction_info as $v_transaction) :
                        ...
                        if($v_transaction->type == 'Income') {
                            $category_info = $this->report_model->check_by(array('income_category_id' => $v_transaction->category_id), 'tbl_income_category')->income_category;
                        } elseif($v_transaction->type == 'Expense') {
                        ...
                        <tr class="custom-tr custom-font-print">
                            ...
                            <?php if($v_transaction->type == 'Income') { ?>
                                <td><?= display_money($balance + $v_transaction->credit, $curency->symbol) ?></td>
                            <?php } elseif($v_transaction->type == 'Expense') { ?>
                                <td><?= display_money($balance - $v_transaction->debit, $curency->symbol) ?></td>
                            <?php } ?>
                            <!-- <td><?= display_money($v_transaction->credit - $v_transaction->debit, $curency->symbol) ?></td> -->
                            <td><?= display_money($v_transaction->total_balance, $curency->symbol) ?></td>
                        </tr>
                        <?php
                        if($v_transaction->type == 'Income') {
                            $balance += $v_transaction->credit;
                        } elseif($v_transaction->type == 'Expense') {
